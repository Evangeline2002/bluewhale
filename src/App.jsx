...
function SectionHeading({ eyebrow, title, desc }) {
  return (
    <div className="mx-auto max-w-2xl text-center">
      <span className="section-label"><Sparkles size={12} /> {eyebrow}</span>
    ...
    <header className={`sticky top-0 z-30 transition-colors ${theme === 'light' ? 'bg-white shadow-sm' : 'border-b border-white/5 bg-night-900/80 backdrop-blur-md'}`}>
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-4">
          <a href="#contact" className={`hidden lg:inline-flex border-2 px-4 py-2 text-xs font-bold uppercase tracking-wider transition ${theme === 'light' ? 'border-brand-500 text-brand-500 hover:bg-brand-500 hover:text-white' : 'btn-primary'}`}>Book Now</a>
          <button
    ...
    <section id="gallery" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
        ...
        {gallery.map((g) => {
          ...
          return (
            <button
              ...
              className={`btn ${active === g.id ? 'btn-primary' : 'btn-outline'}`}
            >
      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {current.items.map((item, index) => (
          <div key={item.name} className="card group overflow-hidden transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl bg-night-850" style={{ borderRadius: index % 2 === 0 ? '2rem 0.5rem 2rem 0.5rem' : '0.5rem 2rem 0.5rem 2rem' }}>
  ...
  return (
    <section id="pricing" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {memberships.map((m) => (
          <div key={m.name} className={`card relative flex flex-col p-6 ${m.popular ? 'border-brand-500/40 shadow-glow' : ''}`}>
            {m.popular && (
            ...
            <a href="#contact" className={`btn mt-6 ${m.popular ? 'btn-primary' : 'btn-outline'}`}>Choose {m.name}</a>
          </div>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {info.map((c) => (
            <div key={c.title} className="card flex items-start gap-4 p-5">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-600/15 text-brand-400"><c.icon size={20} /></div>
              <div>
        ...
        <div className="card p-6 lg:col-span-2">
          {sent ? (
            <div className="flex h-full flex-col items-center justify-center py-12 text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400"><Send size={24} /></div>
              <p className="mt-4 font-display text-lg font-semibold text-white">Message sent!</p>
              <p className="mt-1 text-sm text-slate-500">Thanks for reaching out — we'll get back to you shortly.</p>
            ...
            >
              <div>
                ...
                <input required className="input mt-1" placeholder="Arun Kumar" />
              </div>
              <div>
                <label className="text-xs text-slate-500">Phone</label>
                <input required className="input mt-1" placeholder="98450 12345" />
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea required rows={5} className="input mt-1 resize-none" placeholder="Tell us about your visit, event or group booking..." />
              </div>
              <div className="flex flex-wrap items-center gap-3 sm:col-span-2">
                <button type="submit" className="btn-primary"><Send size={15} /> Send Message</button>
...
export default function App() {
  ...
  return (
    <div className={`${theme === 'light' ? 'theme-light' : ''} flex min-h-screen flex-col`}>
